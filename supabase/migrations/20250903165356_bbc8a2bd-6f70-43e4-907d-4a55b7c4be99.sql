-- Debug and fix the get_discovery_profiles function
-- The issue might be with gender matching logic
...
BEGIN
  ...
  WHERE p.id != user_uuid
    ...
    -- Simplified gender matching - if user is interested in this gender OR has no preferences
    AND (
    ...
    -- Simplified reverse gender matching - if profile is interested in user's gender OR has no preferences  
    AND (
      p.interested_in IS NULL OR 
      array_length(p.interested_in, 1) = 0 OR 
      user_profile.gender = ANY(p.interested_in) OR
      -- For profiles that are interested in common genders but user is 'other'
      (user_profile.gender = 'other' AND (p.interested_in && ARRAY['male'::gender_type, 'female'::gender_type]))
    )
    -- Age filter
