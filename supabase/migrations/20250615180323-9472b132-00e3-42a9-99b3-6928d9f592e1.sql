...
BEGIN
  ...
  WHERE p.id != user_uuid
    ...
    AND (p_filter_relationship_goals IS NULL OR array_length(p_filter_relationship_goals, 1) = 0 OR p.relationship_goals && p_filter_relationship_goals::relationship_goal[])
    -- Filter by distance
    AND (
      p_max_distance <= 0 OR 
